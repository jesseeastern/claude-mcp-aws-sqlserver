...
Configure the constants below (or export as environment variables)
before running:
  python server.py
"""
...
# -- Configuration -- replace with your values (or load from env / secrets) ----
WINDOWS_INSTANCE_ID = "i-XXXXXXXXXXXXXXXXX"   # EC2 instance ID of Windows host
...
SQL_PASSWORD        = "YOUR_SA_PASSWORD_HERE"  # Set this after SQL Server install
# ------------------------------------------------------------------------------
...
def list_instances() -> str:
    ...
    Returns one line per instance:
        Name | InstanceId | State | InstanceType | PublicIp
    """
...
def sql_query(query: str, database: str = "master") -> str:
    ...
    pyodbc + ODBC Driver 17 for SQL Server. No public internet exposure
    is required -- all traffic stays inside the VPC.

    Args:
        query:    Any valid T-SQL statement (SELECT, INSERT, EXEC, ...).
        database: Target database name (default: master).
                  Examples: AdventureWorks2019, TSQL2016, master.
